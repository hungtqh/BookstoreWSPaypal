...
public class CheckoutResource {
	...
	@RequestMapping(value = "/checkoutPaypal", method = RequestMethod.POST)
	public Order checkoutPaypal(@RequestBody HashMap<String, Object> mapper, Principal principal) {
		ObjectMapper om = new ObjectMapper();
		om.disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
		...
		paypal.setAmount(shoppingCart.getGrandTotal().multiply(new BigDecimal(1.06)));
		
		User user = userService.findByUsername(principal.getName());
		Order order = orderService.createOrderWithPaypal(shoppingCart, shippingAddress, paypal, shippingMethod, user);
