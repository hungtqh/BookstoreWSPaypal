...
export class PaymentService {
  ...
  setDefaultPayment (id: number) {
    ...
    let url = this.serverPath + "/payment/paypal";

    let tokenHeader = new Headers({
      'Content-Type': 'application/json',
      'x-auth-token': localStorage.getItem("xAuthToken")
    });
    return this.http.post(url, amount, { headers: tokenHeader });
	}

	confirmPayment(paymentId: string, PayerID: string) {
    let url = this.serverPath + "/payment/confirmPaypal";
    ...
    return this.http.post(url, paymentInfo, { headers: tokenHeader });
  }
}